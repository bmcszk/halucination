...
1.  Upon changes to `docs/requirements.md`, AI Agent MUST create new tasks in **ToDo** state. New tasks MUST ALWAYS be appended to `docs/tasks.md`.
    *   For functional requirements, AI Agent MUST add test scenarios to `docs/scenarios.md` AND create a task to implement E2E tests for EVERY scenario (ensure these align with overall testing strategy in `.windsurf/rules/testing_guidelines.md`).
...
3.  **Task Execution:**
    ...
    *   AI Agent MUST adhere to TDD and proactive checking principles outlined in `.windsurf/rules/testing_guidelines.md` (referenced via `.windsurf/rules/project_guidelines.md`).
...
4.  **Task Completion Criteria:** A task is ONLY considered **Done** when ALL the following conditions are met:
    ...
    *   ALL testing requirements specified in `.windsurf/rules/testing_guidelines.md` (referenced via `.windsurf/rules/project_guidelines.md`) are satisfied for the task's changes.
...
2.  AI Agent MUST frequently push other working changes (even for tasks not fully **Done**) to the remote repository, ONLY AFTER ensuring they pass pre-commit checks (as defined in `.windsurf/rules/testing_guidelines.md` via `.windsurf/rules/project_guidelines.md`) and are committed. This is MANDATORY for work on feature/fix branches (see `.windsurf/rules/requirement_workflow.md`).
