...
- **Documentation Awareness:** When performing tasks, AI Agent MUST ensure all relevant documentation (as specified in the 'Documentation' section, the active PRD, its task tracker, `.windsurf/rules/prd_setup_and_planning.md`, `.windsurf/rules/prd_implementation_and_review.md`, and `.windsurf/rules/testing_*.md`) is considered and updated as necessary.
- **Task Processing:** AI Agent MUST follow `.windsurf/rules/prd_setup_and_planning.md`, `.windsurf/rules/prd_implementation_and_review.md` and the task details within the specific PRD's task tracking file (e.g., `prds/[feature_name]/[feature_name]_prd_task_tracking.md`).
...
- **Truncated Command Output:** If a command's output appears truncated, AI Agent MUST re-run the command redirecting its output to a temporary file, and then analyze the contents of that file to obtain the full output.
